...
public interface HashTagStatsRepository extends JpaRepository<HashTagStats, HashTagCompositeKey> {
    ...
    @Query(value = "select hash_tag from hash_tag_stats where month_of_year =:month order by engagement_count desc limit 10", nativeQuery = true)
    public List<String> fetchTrendingHashTags(int month);
